...
@Database(entities = [AgendaEntry::class, DiaryEntry::class, PendingDelete::class], version = 13, exportSchema = false)
...
abstract class NomyAgendaDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_10_11 = object : Migration(10, 11) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "ALTER TABLE diary_entries ADD COLUMN fontFamily TEXT NOT NULL DEFAULT ''"
                )
                database.execSQL(
                    "ALTER TABLE agenda_entries ADD COLUMN fontFamily TEXT NOT NULL DEFAULT ''"
                )
            }
        }
        ...
        private val MIGRATION_12_13 = object : Migration(12, 13) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "ALTER TABLE agenda_entries ADD COLUMN titleFontSize REAL NOT NULL DEFAULT 0"
                )
                database.execSQL(
                    "ALTER TABLE agenda_entries ADD COLUMN contentFontSize REAL NOT NULL DEFAULT 0"
                )
                database.execSQL(
                    "ALTER TABLE diary_entries ADD COLUMN titleFontSize REAL NOT NULL DEFAULT 0"
                )
                database.execSQL(
                    "ALTER TABLE diary_entries ADD COLUMN contentFontSize REAL NOT NULL DEFAULT 0"
                )
            }
        }
